...
- Generated products declare the optional user-level `all2cf-project` plugin but do not carry its source. One installation owns functional customer-project AI routing across projects. It must not gain visual design ownership, `/factory`, canonical Engine publication, R2 artifact administration, or duplicate official Cloudflare MCP operations. Independent Visual capabilities are delegated through their own plugin and receipt.
- Route All2CF connection, subscription entitlement and paid Starter update requests through local `/maintenance`. Prefer the hosted All2CF MCP with OAuth and a project-scoped connection receipt; use manual receipt import only as fallback. Never expose or commit the project Token. `/all2cf` and `/update` remain compatibility aliases. Native Cloudflare operations stay with official Cloudflare MCP.
- Starter updates must preserve product-owned work through Base/Local/Target classification. Apply Starter-only changes, retain product-only file and dependency changes, and stop on simultaneous changes, unmanaged collisions or modified removals. Never silently overwrite a conflict or use `git reset --hard`; show `/maintenance` diff and require explicit approval before update.
