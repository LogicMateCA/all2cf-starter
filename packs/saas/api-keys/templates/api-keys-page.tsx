...
export function ApiKeysPage() {
  ...
  async function refresh() {
    if (ownerMode === "organization" && !organizationId) {
      setKeys([]);
      return;
    }
    const result = await authClient.apiKey.list({
      query: {
        configId,
        ...(ownerMode === "organization" ? { organizationId } : {}),
        limit: 50,
        offset: 0,
        sortBy: "createdAt",
        sortDirection: "desc",
  ...
  useEffect(() => {
    ...
    if (session?.user) void loadOrganizations();
  }, [isPending, session?.user?.id]);
  useEffect(() => {
    if (session?.user) void refresh();
  }, [session?.user?.id, ownerMode, organizationId]);
  ...
  async function revokeKey(keyId: string) {
    if (
      ...
      return;
    const result = await authClient.apiKey.delete({ configId, keyId });
    if (result.error) {
      ...
      <main className="api-keys-main">
        <header className="api-keys-intro">
          <div>
            ...
            <h1>API Keys</h1>
            <p>
              User keys belong to one account. Organization keys are shared and
              governed by organization roles: owners/admins manage them and
              members receive read-only visibility.
            </p>
          </div>
        ...
        <section className="api-key-create">
          <label>
            ...
            <select
              value={ownerMode}
              onChange={(event) =>
                setOwnerMode(event.target.value as OwnerMode)
              }
            >
        ...
        <section className="api-key-list">
          <div>
            <h2>
              {ownerMode === "organization" ? "Organization keys" : "My keys"}
            </h2>
            ...
            keys.map((key) => (
              <article key={key.id}>
                <div>
                  ...
                  <code>{key.start || key.prefix || "key_"}••••••••</code>
                </div>
                <dl>
                  <div>
                    <dt>Created</dt>
