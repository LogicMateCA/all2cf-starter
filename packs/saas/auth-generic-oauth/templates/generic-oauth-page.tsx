...
export function GenericOAuthPage() {
  ...
  useEffect(() => {
    void fetch("/api/generic-oauth/providers")
      .then(async (response) => {
        if (!response.ok)
  ...
  return (
    <main className="generic-oauth">
      <section>
        <span>Enterprise sign-in</span>
        <h1>Choose your identity provider</h1>
        <p>
          Each connection is configured by the project operator and uses PKCE
          with a fixed provider identity.
        </p>
        <div>
