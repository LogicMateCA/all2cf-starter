...
export function PhoneAuthPage() {
  ...
  async function signIn(event: FormEvent) {
    ...
    setBusy(false);
    if (result.error) setError("Phone number or password is incorrect.");
  ...
  async function send() {
    ...
    setBusy(false);
    if (result.error) setError("Unable to send a verification code.");
    else setSent(true);
  }
  async function verify() {
    ...
    setBusy(false);
    if (result.error) setError("The code is invalid or expired.");
  ...
  return (
    <main className="phone-auth">
      <section>
        ...
        <h1>{signedIn ? "Add or change your phone" : "Sign in by phone"}</h1>
        {signedIn ? (
          <div className="phone-flow">
            <p>
              A six-digit code is delivered by the selected Twilio provider.
              Numbers must use E.164 format.
            </p>
            <Input
              type="tel"
              autoComplete="tel"
            ...
            {sent ? (
              ...
              <Button disabled={busy || !phone} onClick={() => void send()}>
