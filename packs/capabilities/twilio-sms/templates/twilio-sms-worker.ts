...
export async function sendTwilioSms(
  ...
  if (!e164.test(to) || !e164.test(env.TWILIO_FROM)) throw new RangeError("Twilio numbers must use E.164 format.");
  if (!body || body.length > 1_000) throw new RangeError("SMS body must contain 1-1000 characters.");
  if (!kind || kind.length > 64 || !idempotencyKey || idempotencyKey.length > 128) throw new RangeError("SMS kind or idempotency key is invalid.");
  if (!accountSid.test(env.TWILIO_ACCOUNT_SID) || !apiKeySid.test(env.TWILIO_API_KEY) || !env.TWILIO_API_SECRET)
  ...
  const inserted = await database.query(
    ...
    [id, idempotencyKey, input.actorUserId || null, kind, await recipientHash(to), to.slice(-4)],
  );
  if (!inserted.rows[0]) {
    const existing = await database.query(`select id, provider_sid, status, error_code from app_sms_delivery where idempotency_key = $1`, [idempotencyKey]);
  ...
  const accepted = response.ok && Boolean(payload.sid) && new Set(["queued", "accepted", "sending", "sent"]).has(String(payload.status));
  ...
  await database.query(
    ...
    [id, payload.sid || null, status, payload.code ? String(payload.code) : null],
  );
  if (!accepted) throw new Error(payload.message || `Twilio returned HTTP ${response.status}`);
  return { duplicate: false, id, providerSid: payload.sid!, status };
}

const feature = new Hono<{ Bindings: AuthRuntimeEnv }>();

feature.post("/api/admin/sms/test", (c) => withRequestAuth(c.env, c.executionCtx, async (auth, database) => {
  ...
  try {
    ...
    if (error instanceof RangeError)
      return c.json({ error: { code: "INVALID_SMS", message: error.message } }, 422);
    throw error;
  }
}));
