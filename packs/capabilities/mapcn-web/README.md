...
The adapted baseline intentionally keeps only the common product needs: lazy MapLibre loading, an explicit style contract, accessible markers, built-in navigation controls, loading/error/unsupported states, theme-aware blank fallback, and complete cleanup. Routes and the `maplibre-gl` dependency enter `apps/web` only while `capability.mapcn-web` is selected and materialized.
