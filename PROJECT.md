...
- `packs/` contains optional assembler inputs for MapCN Web, Better Auth Organizations, and Stripe Billing. `npm run starter:materialize` plans, `starter:materialize:apply` writes, and `starter:materialize:check` proves that selected files, dependencies, routes, auth-plugin registries, lifecycle state, and the ownership receipt agree.
