...
- Routine AI work starts from `AGENT_MAP.md` plus task-scoped `ai:context`, not the whole repository. Full Catalog, Module and Change-Spec context is reserved for first initialization, whole-project architecture and release/cutover audit.
...
- The reusable Starter has one fixed `editorial@2.2.0` fallback: a restrained white/light-gray and black/dark-gray SaaS foundation. Factory and generated Setup do not expose the donor inventory or rotate this baseline; later visual work belongs to the independent Visual plugin and still requires explicit acceptance, materialization and release.
- Universal visual intelligence is an independent product at `/opt/1panel/apps/visual`. Starter pins stable `starter-integration@1.0.1`, bundles only the functional `all2cf-project` plugin, recommends the external `visual-design` plugin, and remains fully usable through its owned fallback when Visual is unavailable.
- Its structured database policy fixes the PostgreSQL engine, SQL-first access, an empty initial state, the final selected-pack schema baseline, and no existing-data compatibility scope. `/setup` independently selects Development and Production transport as native PostgreSQL through Hyperdrive or CFPG through `@all2cf/database-connect`; changing one environment never changes the other. AI must not infer migration or backfill work for a new Starter project.
- Product code has exactly two data-layer choices: SQL-first (default) and optional Drizzle. Both use the same `pg` runtime, SQL semantics and PostgreSQL schema; Hyperdrive or CFPG only changes the connection transport. Starter and Pack migrations never become ORM-owned.
...
- Its automated completion contract rejects selectable external options without an official setup entry or verification action and pins high-risk credential names to the actual Setup/release implementation, preventing AI from following stale Catalog variables.
...
- A selected storage release is accepted only after the deployed adapter passes a release-authenticated exact-byte put/get/delete round trip. The release controller records the exact R2/Vectorize/Queue identities observed after Wrangler deployment, including resources Wrangler provisioned automatically.
...
- Product search defaults to `None`; PostgreSQL is the simplest executable choice, while Vectorize is a separate optional Pack for embedding similarity/RAG with immutable environment-specific index configuration.
- Native push defaults to `None`. Expo Push is an executable optional Pack with separate EAS project IDs, SDK-compatible native configuration, user-owned device tokens and ticket evidence; physical-device delivery is still a release gate.
- SMS also defaults to `None`. Twilio SMS is an independent server-only Pack using production API-key authentication, SQL idempotency, E.164 validation and provider SID evidence; it is not silently reused as authentication second factor.
- Image optimization defaults to `None`. Cloudflare Images is an independent Worker Binding Pack for bounded transformation; it does not replace the selected Object Storage Provider or silently enable video streaming.
- Video streaming defaults to `None`. Cloudflare Stream is a separate Pack for user-owned direct upload, signed processing webhooks and public playback; signed/private playback is not offered until its token path is complete.
- Scheduled work defaults to `None`. Cloudflare Cron is an optional UTC schedule/event/heartbeat Pack; Queues remains independently owned by capabilities such as Outgoing Webhooks.
- Durable multi-step jobs also default to `None`. Cloudflare Workflows is a separate class/Binding/resource Pack with optional binding schedules and explicit remote resource deletion on a later deselected release.
- Realtime state also defaults to `None`. The executable Cloudflare choice combines a SQLite-backed Durable Object per room with authenticated hibernatable WebSockets, bounded messages and a fixed Admin round-trip; deselection removes routes/Binding/code without automatically deleting stored namespace data.
...
- CFPG is an explicit per-environment database transport choice, not a permanent dependency. Setup validates the exact All2CF connection command only for an environment that selects it and stores its non-secret database ID plus immutable install descriptor. Materialization adds `@all2cf/database-connect@0.2.0-rc.2` when at least one environment needs it, aliases `pg`, replaces Hyperdrive with `ALL2CF_DATABASE` only in the selected Wrangler configuration, and owns reversal in its receipt. When both environments select CFPG, they require distinct database IDs. Older generated Blueprints retain their global Provider fallback.
