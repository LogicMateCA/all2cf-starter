...
- Universal visual intelligence is an independent product at `/opt/1panel/apps/visual`. Starter pins draft `starter-integration@1.0.1`, bundles only the functional `all2cf-project` plugin, recommends the external `visual-design` plugin, and remains fully usable through its owned fallback when Visual is unavailable.
