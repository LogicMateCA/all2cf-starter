...
- Local Android commands are `mobile:build:android:dev|preview|production`; they generate native source with Expo CNG, then use Gradle. Preview/Production require the four `ANDROID_RELEASE_*` signing values and never use the debug key. Local Mac commands are `mobile:build:ios:dev|preview|production`; connected-Mac equivalents insert `:mac:`. Preview/Production require `IOS_EXPORT_OPTIONS_PLIST` and produce an Xcode archive/export.
- On the canonical Linux/1Panel source, run Android commands through `docker compose --profile mobile run --rm starter-android-builder ...`; the normal Starter container intentionally has no JDK/SDK. Other generated projects may run the same npm commands directly when their host already has Java and Android SDK.
