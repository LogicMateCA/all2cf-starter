...
For an older project or copied module, run `npm run feature:adopt -- --root <path>`. Review `.ai/feature-adoption-candidates.json`; never bulk-apply inferred ownership. Register each accepted candidate through `feature:add`, then move or map its real paths without rewriting its business behavior merely to fit Starter conventions.
