...
3. Run all repository commands in the project container. Use `npm run verify` for a build-only request; it checks the committed Development and Production Worker types without rewriting them. After a Binding/config change, run `npm run cf:types` explicitly and commit the reviewed result before verification. Use `npm run release:dev` for Development and `npm run release:production` for Production. Do not bypass either clean-worktree check, verification, artifact-parity, or live-route gates in `scripts/starterctl.mjs`.
   Run ordinary checks and Development releases in `starter-dev`. Use `docker compose --profile ops run --rm starter-ops ...` only when the command needs Docker provisioning or Production SSH access.
...
`npm run starter:provision:dev` and `npm run starter:provision:production` create or reconcile only the named environment's database, Cloudflare connectivity resources, and Queues declared by selected materializer packs. `npm run starter:provision` retains the explicit all-environment initialization path. Provisioning preserves receipt-owned bindings and secret-name requirements, but secret values remain environment-specific inputs. It is not part of an ordinary release; run the scoped command only for initial setup or an explicit infrastructure reconciliation, and inspect target identities before mutation. A Development release must never invoke Production provisioning implicitly.
