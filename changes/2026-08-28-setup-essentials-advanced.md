...
Product intent is reduced to the single Product identity brief. The compatibility machine block derives from that brief and explicit Organizations/Billing selections; users no longer repeat a brief, audience, core-object, tenancy or charging form. Core objects move to product-domain implementation. The previous inferred proposal cannot auto-select modules.
