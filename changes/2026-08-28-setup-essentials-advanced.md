...
Setup has six steps: Product, SaaS, Providers, Pages, Design and Review. The duplicate Capabilities step is removed because each former capability is already controlled by a Provider `None`/implementation choice or SQL-first/Drizzle. Provider navigation now has only Essentials and Advanced. Essentials contains native PostgreSQL with SQL-first/Drizzle, object storage, social sign-in, authentication email and billing. Advanced contains maps, Turnstile, AI/search, native push, SMS, media, Cron, Workflows, realtime, release platforms and the complete reference Catalog.
