...
- Optional source templates live under `packs/` and are excluded from application imports. Generated client route, Worker route, and auth-plugin registries have no optional imports when no pack is selected. A capability route that sits outside the permanent application route families must explicitly declare `workerFirst`; materialization adds the exact route to both Wrangler environments and the Worker asset-shell registry, and removal deletes both. MapCN adds only its map route/runtime; Organization adds its official server/client plugin adapters, empty-baseline SQL and product routes; Stripe adds its aligned SDK/plugin, empty-baseline SQL, billing route and release-secret contract. Deselection removes only receipt-matching assets.
- Presets compose only executable Catalog IDs rather than duplicating implementations. Basic Product contains the required core; Team SaaS adds Better Auth organizations and Stripe; Custom records explicit deviations. API Platform remains a visible planned pack until API keys, signed webhooks, usage, developer docs, and their verification flow are implemented.
