...
- Better Auth 1.7.1 owns identity endpoints in the repository. It uses its built-in PostgreSQL/Kysely adapter through request-scoped Hyperdrive pools, host-only Web cookies, official Expo deep-link/SecureStore plugins, database rate limits, and SQL-first migrations. The shared environments remain on their prior release until the Development test rows are explicitly cleared, both empty schemas are recreated from the new baseline, and release is authorized.
