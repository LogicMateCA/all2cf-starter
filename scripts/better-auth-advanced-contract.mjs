...
assert.doesNotMatch(await read("packs/saas/api-keys/templates/0007_api_keys.sql"), /reference_id[^\n]+references\s+"app_user"/u);
const apiKeyOwnershipMigration = await read("packs/saas/api-keys/templates/0008_api_key_reference_ownership.sql");
assert.match(apiKeyOwnershipMigration, /config_id = 'user-keys'[\s\S]*config_id = 'org-keys'/u);
