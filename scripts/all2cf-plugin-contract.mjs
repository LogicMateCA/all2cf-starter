...
const projectPlugins = JSON.parse(await readFile(path.join(root, ".ai/plugins.json"), "utf8"));
if (manifest.name !== "all2cf-project") failures.push("Plugin name must be all2cf-project");
if (projectPlugins.plugins?.find(({ id }) => id === "all2cf-project")?.version !== manifest.version) failures.push("Generated-project plugin declaration must match the plugin source version");
...
const expected = new Set(["cloudflare-release", "expo-release", "project-adoption", "project-change", "project-doctor", "project-onboarding", "runtime-upgrade", "starter-update"]);
