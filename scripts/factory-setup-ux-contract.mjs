...
const [setup, styles, factory, providers, project] = await Promise.all([
  ...
  read("scripts/starter-factory.mjs"),
  read("catalog/providers.json").then(JSON.parse),
  read("PROJECT.md"),
]);
...
assert.match(
  setup,
  /\[\s*"essentials",\s*"Essentials"\s*\],\s*\[\s*"advanced",\s*"Advanced"\s*\]/u,
);
...
for (const removed of [
  ...
  "Tenant model",
  "Charging model",
  "Intent-derived module proposal",
])
...
for (const field of [
  "STARTER_PRODUCTION_STRIPE_SECRET_KEY",
  "STARTER_PRODUCTION_STRIPE_PUBLISHABLE_KEY",
  "STARTER_PRODUCTION_STRIPE_WEBHOOK_SECRET",
  "STARTER_PRODUCTION_STRIPE_PRICE_PRO",
])
...
assert.match(
  styles,
  /\.provider-option input \{ position: absolute; opacity: 0/u,
);
assert.doesNotMatch(
  styles,
  /\.setup-(?:main|stack|panel)[^{]*\{[^}]*overflow-y:\s*(?:auto|scroll)/u,
);
assert.match(factory, /applyAutomaticProviderDefaults/u);
assert.match(
  factory,
  /blueprint\.providers\.push\.provider = nativeMobile \? "expo-push" : "none"/u,
);
assert.match(
  project,
  /user who can list an organization-scoped Draft can request its generation/u,
);
assert.match(
  project,
  /authorization failures are service-credential drift, never user `Unauthorized`/u,
);
...
assert.deepEqual(
  maps.options
    .filter(({ id }) =>
      ["none", "mapcn", "mapbox", "google-places"].includes(id),
    )
    .map(({ id }) => id),
  ["none", "mapcn", "mapbox", "google-places"],
);
assert.equal(maps.options.find(({ id }) => id === "mapcn").selectable, true);
assert.equal(maps.options.find(({ id }) => id === "mapbox").selectable, false);
assert.equal(
  maps.options.find(({ id }) => id === "google-places").selectable,
  false,
);
assert.equal(
  providers.categories
    .find(({ id }) => id === "database")
    .options.find(({ id }) => id === "cfpg").selectable,
  false,
);
assert(
  !providers.categories
    .find(({ id }) => id === "notification-channels")
    .options.some(({ id }) => id === "expo-push" || id === "twilio-sms"),
);
  ...
  JSON.stringify(
    {
      ...
      mobilePushDefault: true,
      smsCategory: "provider",
      objectStorageCategory: "provider",
      mapsCategory: "provider",
    },
    null,
    2,
  ),
