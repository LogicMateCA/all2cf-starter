...
try {
  ...
  const result = spawnSync("npx", ["--yes", `auth@${betterAuthVersion}`, "generate", "--config", "workers/app/auth.cli.ts", "--output", path.relative(root, output), "--yes"], {
    cwd: root,
    encoding: "utf8",
    env: { ...process.env, DATABASE_URL: scratchUrl.toString(), BETTER_AUTH_SECRET: values.get("BETTER_AUTH_SECRET") },
  });
  ...
  await admin.query(`drop database if exists "${scratchDatabase}" with (force)`).catch(() => undefined);
  await admin.end();
}
