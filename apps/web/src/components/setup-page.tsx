...
function PackChoice({ pack, selection, type, onChange }: { pack: Pack; selection: Selection; type: "radio" | "checkbox"; onChange: (selected: boolean) => void }) {
  ...
  return <label className={selection.lifecycle.selected ? "pack-choice selected" : "pack-choice"}>
    <input type={type} name={type === "radio" ? "design-profile" : pack.id} checked={selection.lifecycle.selected} disabled={required} onChange={(event) => onChange(event.target.checked)} />
    <span className="pack-choice-main"><span><strong>{pack.name}</strong>{required ? <small>Required</small> : null}</span><p>{pack.provides.slice(0, 3).join(", ")}</p><small>{pack.targets.join(", ")} / {pack.status}</small>{pack.requires.length ? <small>Requires: {pack.requires.join(", ")}</small> : null}{pack.conflicts.length ? <small>Conflicts: {pack.conflicts.join(", ")}</small> : null}</span>
