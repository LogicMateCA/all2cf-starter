...
type SetupPayload = { blueprint: Blueprint; catalog: { catalogVersion: string; presets: Preset[]; packs: Pack[] }; config: StarterConfig };
...
function PackChoice({ pack, selection, type, onChange }: { pack: Pack; selection: Selection; type: "radio" | "checkbox"; onChange: (selected: boolean) => void }) {
  ...
  return <label className={selection.lifecycle.selected ? "pack-choice selected" : "pack-choice"}>
    ...
    <span className="pack-choice-main"><span><strong>{pack.name}</strong>{required ? <small>Required</small> : null}</span><p>{pack.provides.slice(0, 3).join(", ")}</p><small>{pack.targets.join(", ")} / {pack.status}</small></span>
...
export function SetupPage() {
  ...
  const applyPreset = (preset: Preset) => updateBlueprint((blueprint) => {
    ...
    return { ...blueprint, preset: preset.id, selections };
  });
  ...
  return <div className="setup-shell">
    ...
    <div className="setup-layout">
      ...
      <main className="setup-main">
        ...
        {currentStep.id === "design" ? <div className="pack-grid">{packsFor("design").map((pack) => <PackChoice key={pack.id} pack={pack} selection={selectionFor(pack)} type="radio" onChange={(selected) => setPackSelected(pack, selected)} />)}</div> : null}
