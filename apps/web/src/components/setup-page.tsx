...
type MediaPolicy = {
  images: {
    provider: "none" | "cloudflare-images";
    maxInputBytes: number;
    defaultFormat: "image/webp" | "image/avif" | "image/jpeg" | "image/png";
  };
  stream: {
    provider: "none" | "cloudflare-stream";
...
const providerSecretFields = {
  ...
  apple: [
    ...
    {
      name: "APPLE_PRIVATE_KEY_BASE64",
      label: "P8 private key (base64)",
      secret: true,
    },
    {
  ...
  microsoft: [
    { name: "MICROSOFT_CLIENT_ID", label: "Client ID", secret: false },
    { name: "MICROSOFT_CLIENT_SECRET", label: "Client secret", secret: true },
  ],
  discord: [
    { name: "DISCORD_CLIENT_ID", label: "Client ID", secret: false },
    { name: "DISCORD_CLIENT_SECRET", label: "Client secret", secret: true },
  ],
  facebook: [
    { name: "FACEBOOK_CLIENT_ID", label: "Client ID", secret: false },
    { name: "FACEBOOK_CLIENT_SECRET", label: "Client secret", secret: true },
  ],
  linkedin: [
    { name: "LINKEDIN_CLIENT_ID", label: "Client ID", secret: false },
  ...
  stripe: [
    {
      ...
      label: "Development · Test secret or restricted key",
      secret: true,
    },
    {
    ...
    {
      name: "STARTER_PRODUCTION_STRIPE_SECRET_KEY",
      label: "Production · Live secret or restricted key",
      secret: true,
    },
    {
      name: "STARTER_PRODUCTION_STRIPE_PUBLISHABLE_KEY",
      label: "Live publishable key",
      secret: false,
    },
    {
      name: "STARTER_PRODUCTION_STRIPE_WEBHOOK_SECRET",
      label: "Live webhook signing secret",
      secret: true,
    },
    {
      name: "STARTER_PRODUCTION_STRIPE_PRICE_PRO",
      label: "Live Pro Price ID",
      secret: false,
    },
  ],
  polar: [
    { name: "POLAR_ACCESS_TOKEN", label: "Access token", secret: true },
    { name: "POLAR_WEBHOOK_SECRET", label: "Webhook secret", secret: true },
    { name: "POLAR_PRODUCT_PRO", label: "Pro product ID", secret: false },
  ],
  ...
  turnstile: [
    ...
    {
      name: "STARTER_PRODUCTION_TURNSTILE_SECRET_KEY",
  ...
  "expo-push": [
    ...
    {
      name: "STARTER_PRODUCTION_EXPO_PUSH_ACCESS_TOKEN",
  ...
  "twilio-sms": [
    {
      ...
      label: "Development Account SID",
      secret: false,
    },
    { name: "TWILIO_API_KEY", label: "Development API Key SID", secret: false },
    {
      name: "TWILIO_API_SECRET",
      label: "Development API Secret",
      secret: true,
    },
    { name: "TWILIO_FROM", label: "Development sender (E.164)", secret: false },
    {
      name: "STARTER_PRODUCTION_TWILIO_ACCOUNT_SID",
      label: "Production Account SID",
      secret: false,
    },
    {
      name: "STARTER_PRODUCTION_TWILIO_API_KEY",
      label: "Production API Key SID",
      secret: false,
    },
    {
      name: "STARTER_PRODUCTION_TWILIO_API_SECRET",
      label: "Production API Secret",
      secret: true,
    },
    {
      name: "STARTER_PRODUCTION_TWILIO_FROM",
      label: "Production sender (E.164)",
      secret: false,
    },
  ],
  "cloudflare-stream": [
    {
      ...
      label: "Development Stream token",
      secret: true,
    },
    {
      name: "STREAM_WEBHOOK_SECRET",
      label: "Development webhook secret",
      secret: true,
    },
    {
      name: "STARTER_PRODUCTION_CLOUDFLARE_STREAM_TOKEN",
      label: "Production Stream token",
      secret: true,
    },
    {
      name: "STARTER_PRODUCTION_STREAM_WEBHOOK_SECRET",
      label: "Production webhook secret",
      secret: true,
    },
  ],
  "cloudflare-release": [
  ...
  "mobile-local-build": [
    ...
    {
      name: "MOBILE_ANDROID_ARCHITECTURES",
      label: "Development ABIs (default arm64-v8a)",
      secret: false,
    },
    {
      name: "ANDROID_RELEASE_KEYSTORE",
      label: "Android upload keystore path",
      secret: false,
    },
    {
      name: "ANDROID_RELEASE_STORE_PASSWORD",
      label: "Android keystore password",
      secret: true,
    },
    {
      name: "ANDROID_RELEASE_KEY_ALIAS",
      label: "Android key alias",
      secret: false,
    },
    {
      name: "ANDROID_RELEASE_KEY_PASSWORD",
      label: "Android key password",
      secret: true,
    },
    { name: "MOBILE_MAC_HOST", label: "Connected Mac SSH host", secret: false },
    {
      name: "MOBILE_MAC_PROJECT_ROOT",
      label: "Project path on Mac",
      secret: false,
    },
    {
      name: "MOBILE_MAC_SSH_KEY_PATH",
      label: "Mac SSH key path",
      secret: false,
    },
    {
      name: "IOS_EXPORT_OPTIONS_PLIST",
      label: "iOS ExportOptions.plist path",
      secret: false,
    },
  ],
  "apple-app-store": [
...
const identityPackConflicts: Record<string, string[]> = {
  "saas.auth-oauth-provider": ["saas.auth-mcp"],
  "saas.auth-mcp": ["saas.auth-oauth-provider"],
};
  ...
  return (
    <div className="provider-credentials">
      <div className="provider-credential-state">
        <span
          className={state.configured ? "status configured" : "status deferred"}
        >
      ...
      <div
        className="provider-resource-links"
        aria-label={`${provider} setup resources`}
      >
      ...
      {expanded ? (
        <div className="provider-secret-fields">
          ...
          <div className="provider-secret-actions">
            ...
            <small>
              Non-empty values are saved only to the project-local development
              environment.
            </small>
          </div>
        </div>
...
export function SetupPage() {
  ...
  const [providerTab, setProviderTab] = useState<"essentials" | "advanced">(
    "essentials",
  );
  ...
  >({});
  const [providerEditors, setProviderEditors] = useState<
    Partial<Record<keyof typeof providerSecretFields, boolean>>
  >({});
  const [testRecipient, setTestRecipient] = useState("");
  const [providerTest, setProviderTest] = useState<ProviderTestState>({
    status: "idle",
  });
  const [visualTest, setVisualTest] = useState<ProviderTestState>({
    status: "idle",
  });
  const [turnstileToken, setTurnstileToken] = useState("");
  const [turnstileTest, setTurnstileTest] = useState<ProviderTestState>({
    status: "idle",
  });
  const [workersAiTest, setWorkersAiTest] = useState<ProviderTestState>({
    status: "idle",
  });
  const [vectorizeTest, setVectorizeTest] = useState<ProviderTestState>({
    status: "idle",
  });
  const [expoPushToken, setExpoPushToken] = useState("");
  const [expoPushTest, setExpoPushTest] = useState<ProviderTestState>({
    status: "idle",
  });
  const [smsRecipient, setSmsRecipient] = useState("");
  const [smsTest, setSmsTest] = useState<ProviderTestState>({ status: "idle" });
  const [streamTest, setStreamTest] = useState<ProviderTestState>({
    status: "idle",
  });
  const [releaseTests, setReleaseTests] = useState<
    Partial<Record<ReleaseProvider, ProviderTestState>>
  >({});
  const [cfpgCommands, setCfpgCommands] = useState({
    development: "",
    production: "",
  });
  ...
  useEffect(() => {
    void fetch("/__starter/setup", { headers: { Accept: "application/json" } })
      ...
      .then((next) => {
        ...
        setCfpgCommands({
          development:
            next.blueprint.providers.database.cfpg?.development
              ?.connectCommand || "",
          production:
            next.blueprint.providers.database.cfpg?.production
              ?.connectCommand || "",
        });
        const resumedSave = sessionStorage.getItem("starter.setup.savePending");
        if (next.blueprint.setup.status === "ready" || resumedSave)
          setSaveState("saved");
  ...
  const billingProvider =
    payload?.blueprint.providers.billing === "better-auth-polar"
      ? "polar"
      : payload?.blueprint.providers.billing === "better-auth-autumn"
        ? "autumn"
        : "stripe";
  const billingSelected = selectedPacks.some(({ id }) =>
    id.startsWith("saas.billing-"),
  );
  const nativeMobileSelected =
    payload?.blueprint.project.platforms.some(
  ...
  const runEmailProviderTest = async () => {
    ...
    try {
      ...
      if (!response.ok || !result.result)
        throw new Error(
          result.error || `Provider test returned HTTP ${response.status}.`,
        );
      setProviderTest({
        status: "success",
        provider,
  ...
  const runVisualDiscoveryTest = async () => {
    ...
    try {
      const response = await fetch("/__starter/visual-discovery", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
      ...
      const result = (await response.json()) as {
        error?: string;
        status?: string;
        discovery?: { serviceVersion?: string; protocolVersion?: string };
        warnings?: string[];
      };
      if (!response.ok)
        throw new Error(
          result.error || `Visual discovery returned HTTP ${response.status}.`,
        );
      if (result.status !== "configured")
        throw new Error(
          result.warnings?.join(" ") ||
            "Visual discovery is unavailable; Starter baseline remains active.",
        );
      setVisualTest({
        status: "success",
        provider: "visual-design",
        message: `Visual discovery is compatible${result.discovery?.serviceVersion ? `; service ${result.discovery.serviceVersion}` : ""}${result.discovery?.protocolVersion ? `; protocol ${result.discovery.protocolVersion}` : ""}.`,
      });
    } catch (error) {
      setVisualTest({
        status: "error",
  ...
  const runTurnstileTest = async () => {
    ...
    try {
      ...
      if (!response.ok || !result.result?.verified)
        throw new Error(
          result.error || `Turnstile test returned HTTP ${response.status}.`,
        );
      setTurnstileTest({
        status: "success",
        provider: "turnstile",
        message: `Siteverify accepted the challenge for ${result.result.hostname}; action: ${result.result.action}.`,
      });
      setTurnstileToken("");
    } catch (error) {
      setTurnstileTest({
  ...
  const runWorkersAiTest = async () => {
    ...
    try {
      ...
      if (!response.ok || !result.result)
        throw new Error(
          result.error || `Workers AI test returned HTTP ${response.status}.`,
        );
      setWorkersAiTest({
        status: "success",
        provider: "workers-ai",
        message: `${result.result.model} responded${result.result.gatewayId ? ` through ${result.result.gatewayId}` : " directly"}: ${result.result.response || "response received"}`,
      });
    } catch (error) {
      setWorkersAiTest({
        status: "error",
  ...
  const runVectorizeTest = async () => {
    ...
    try {
      ...
      if (!response.ok || !result.result)
        throw new Error(
          result.error || `Vectorize test returned HTTP ${response.status}.`,
        );
      setVectorizeTest({
        status: "success",
        provider: "vectorize",
        message: `${result.result.indexName} accepted, queried and deleted a ${result.result.dimensions}-dimension test vector (${result.result.metric}); score: ${result.result.match.score ?? "n/a"}.`,
      });
    } catch (error) {
      setVectorizeTest({
        status: "error",
    ...
    try {
      const response = await fetch("/__starter/provider-test", {
        ...
        body: JSON.stringify({
          ...
          token: expoPushToken,
          accessTokenRequired:
            payload.blueprint.providers.push.accessTokenRequired,
          providerSecrets,
        }),
      });
      const result = (await response.json()) as {
        error?: string;
        result?: { ticketId: string; status: string };
      };
      if (!response.ok || !result.result)
        throw new Error(
          result.error || `Expo Push test returned HTTP ${response.status}.`,
        );
      setExpoPushTest({
        status: "success",
        provider: "expo-push",
        message: `Expo accepted the Development push ticket ${result.result.ticketId}. Device delivery and receipt still require physical-device confirmation.`,
      });
    } catch (error) {
      setExpoPushTest({
        status: "error",
    ...
    try {
      const response = await fetch("/__starter/provider-test", {
        ...
        body: JSON.stringify({
          ...
          apiBaseUrl: payload.blueprint.providers.sms.development.apiBaseUrl,
          providerSecrets,
        }),
      });
      const result = (await response.json()) as {
      ...
      if (!response.ok || !result.result)
        throw new Error(
          result.error || `Twilio test returned HTTP ${response.status}.`,
        );
      setSmsTest({
        status: "success",
        provider: "twilio-sms",
        message: `Twilio accepted ${result.result.providerSid} for ••••${result.result.recipientLast4}; initial status: ${result.result.status}. Carrier delivery remains a separate receipt/status check.`,
      });
    } catch (error) {
      setSmsTest({
        status: "error",
  ...
  const runStreamTest = async () => {
    ...
    try {
      ...
      const result = (await response.json()) as {
        error?: string;
        result?: { uid: string; deleted: boolean };
      };
      if (!response.ok || !result.result?.deleted)
        throw new Error(
          result.error || `Stream test returned HTTP ${response.status}.`,
        );
      setStreamTest({
        status: "success",
        provider: "cloudflare-stream",
        message: `Stream created direct upload ${result.result.uid} and deleted the unused draft successfully.`,
      });
    } catch (error) {
      setStreamTest({
        status: "error",
  ...
  const runReleasePlatformTest = async (provider: ReleaseProvider) => {
    setReleaseTests((current) => ({
      ...current,
      [provider]: { status: "testing", provider },
    }));
    try {
      const response = await fetch("/__starter/provider-test", {
        method: "POST",
        ...
        body: JSON.stringify({ provider, providerSecrets }),
      });
      const result = (await response.json()) as {
        error?: string;
        result?: { verified: boolean; identity: string };
      };
      if (!response.ok || !result.result?.verified)
        throw new Error(
      ...
      setReleaseTests((current) => ({
        ...
        [provider]: {
          status: "success",
          provider,
          message: `Verified read-only access to ${result.result?.identity}.`,
        },
      }));
    } catch (error) {
      setReleaseTests((current) => ({
  ...
  const setPackSelected = (pack: Pack, selected: boolean) =>
    updateBlueprint((blueprint) => {
      ...
      if (
        !selected &&
        Object.entries(identityPackDependencies).some(
          ([dependent, dependencies]) =>
            selectedIds.has(dependent) && dependencies.includes(pack.id),
        )
      )
        return blueprint;
      const dependencies = new Set(
        selected ? identityPackDependencies[pack.id] || [] : [],
      );
      const conflicts = new Set(
        selected ? identityPackConflicts[pack.id] || [] : [],
      );
      ...
      const nextSelections = Object.fromEntries(
        Object.entries(baseSelections).map(
          ([selectionGroup, groupSelections]) => [
            selectionGroup,
            groupSelections.map((selection) => {
              const implied = dependencies.has(selection.id)
                ? true
                : conflicts.has(selection.id)
                  ? false
                  : selection.lifecycle.selected;
              return implied === selection.lifecycle.selected
                ? selection
                : {
                    ...selection,
                    lifecycle: selectLifecycle(selection.lifecycle, implied),
                  };
            }),
          ],
        ),
        ...
        providers:
          pack.id === "saas.auth-google-one-tap" && selected
            ? {
                ...blueprint.providers,
                socialAuth: [
                  ...new Set([...blueprint.providers.socialAuth, "google"]),
                ],
              }
            : pack.id === "saas.auth-phone" && selected
              ? {
                  ...blueprint.providers,
                  sms: { ...blueprint.providers.sms, provider: "twilio" },
                }
                    ...
                    : pack.id === "capability.vectorize"
                      ...
                      : pack.id === "capability.expo-push"
                        ? {
                            ...blueprint.providers,
                            push: {
                              ...blueprint.providers.push,
                              provider: selected ? "expo-push" : "none",
                            },
                          }
                        : pack.id === "capability.twilio-sms"
                          ? {
                              ...blueprint.providers,
                              sms: {
                                ...blueprint.providers.sms,
                                provider: selected ? "twilio" : "none",
                              },
                            }
                          : pack.id === "capability.cloudflare-images"
                            ? {
                                ...blueprint.providers,
                                media: {
                                  ...blueprint.providers.media,
                                  images: {
                                    ...blueprint.providers.media.images,
                                    provider: selected
                                      ? "cloudflare-images"
                                      : "none",
                                  },
                            ...
                            : pack.id === "capability.cloudflare-stream"
                              ? {
                                  ...
                                  media: {
                                    ...blueprint.providers.media,
                                    stream: {
                                      ...blueprint.providers.media.stream,
                                      provider: selected
                                        ? "cloudflare-stream"
                                        : "none",
                                    },
                              ...
                              : pack.id === "capability.cron"
                                ? {
                                    ...blueprint.providers,
                                    background: {
                                      ...blueprint.providers.background,
                                      cron: {
                                        ...blueprint.providers.background.cron,
                                        enabled: selected,
                                      },
                                    },
                                  }
  ...
  const setPushProvider = (provider: PushPolicy["provider"]) =>
    updateBlueprint((blueprint) => ({
      ...
      providers: {
        ...blueprint.providers,
        push: { ...blueprint.providers.push, provider },
      },
      selections: {
        ...blueprint.selections,
        capabilities: blueprint.selections.capabilities.map((selection) =>
  ...
  const setBillingCapability = (selected: boolean) =>
    updateBlueprint((blueprint) => {
      const provider = selected
        ? blueprint.providers.billing === "better-auth-polar" ||
          blueprint.providers.billing === "better-auth-autumn"
          ? blueprint.providers.billing
          : "better-auth-stripe"
        : "none";
      const selectedPack =
        provider === "better-auth-polar"
          ? "saas.billing-polar"
          : provider === "better-auth-autumn"
            ? "saas.billing-autumn"
            : "saas.billing-stripe";
      return {
        ...blueprint,
        ...
        selections: {
          ...blueprint.selections,
          saas: blueprint.selections.saas.map((selection) =>
            selection.id.startsWith("saas.billing-")
              ? {
                  ...selection,
                  lifecycle: selectLifecycle(
                    selection.lifecycle,
                    selected && selection.id === selectedPack,
                  ),
                }
              : selection,
          ),
  ...
  const setSmsProvider = (provider: SmsPolicy["provider"]) =>
    updateBlueprint((blueprint) => ({
      ...
      providers: {
        ...blueprint.providers,
        sms: { ...blueprint.providers.sms, provider },
      },
      selections: {
        ...blueprint.selections,
        capabilities: blueprint.selections.capabilities.map((selection) =>
  ...
  const setImagesProvider = (provider: MediaPolicy["images"]["provider"]) =>
    updateBlueprint((blueprint) => ({
      ...
      providers: {
        ...
        media: {
          ...blueprint.providers.media,
          images: { ...blueprint.providers.media.images, provider },
        },
      },
      selections: {
        ...blueprint.selections,
  ...
  const setStreamProvider = (provider: MediaPolicy["stream"]["provider"]) =>
    updateBlueprint((blueprint) => ({
      ...
      providers: {
        ...
        media: {
          ...blueprint.providers.media,
          stream: { ...blueprint.providers.media.stream, provider },
        },
      },
      selections: {
        ...blueprint.selections,
        capabilities: blueprint.selections.capabilities.map((selection) =>
          selection.id === "capability.cloudflare-stream"
            ? {
                ...selection,
                lifecycle: selectLifecycle(
                  selection.lifecycle,
  ...
  const setCronEnabled = (enabled: boolean) =>
    updateBlueprint((blueprint) => ({
      ...
      providers: {
        ...
        background: {
          ...blueprint.providers.background,
          cron: { ...blueprint.providers.background.cron, enabled },
        },
      },
      selections: {
        ...blueprint.selections,
        capabilities: blueprint.selections.capabilities.map((selection) =>
          selection.id === "capability.cron"
            ? {
                ...selection,
                lifecycle: selectLifecycle(selection.lifecycle, enabled),
              }
            : selection,
        ),
      },
    ...
    updateBlueprint((blueprint) => ({
      ...
      providers: {
        ...
        background: {
          ...blueprint.providers.background,
          workflow: {
            ...blueprint.providers.background.workflow,
            enabled,
      ...
      selections: {
        ...blueprint.selections,
        capabilities: blueprint.selections.capabilities.map((selection) =>
          selection.id === "capability.workflows"
            ? {
                ...selection,
                lifecycle: selectLifecycle(selection.lifecycle, enabled),
              }
  ...
  const setRealtimeEnabled = (enabled: boolean) =>
    updateBlueprint((blueprint) => ({
      ...
      selections: {
        ...blueprint.selections,
        capabilities: blueprint.selections.capabilities.map((selection) =>
          selection.id === "capability.durable-objects"
            ? {
                ...selection,
                lifecycle: selectLifecycle(selection.lifecycle, enabled),
              }
  ...
  const mapsCategory = payload.providerCatalog.categories.find(
    ({ id }) => id === "maps",
  )!;
  const mapPack = payload.catalog.packs.find(
    ({ id }) => id === "capability.mapcn-web",
  )!;
  const selectedMapProvider = selectionFor(mapPack).lifecycle.selected
    ? "mapcn"
    : "none";
  ...
  const save = async ({
    ...
    const completedSteps = steps
      .slice(
        ...
        finish
          ...
          : Math.min(
              steps.length - 1,
              Math.max(stepIndex + 1, targetStepIndex),
            ),
      )
      .map(({ id }) => id);
    const blueprint: Blueprint = {
      ...
      productIntent: {
        ...
        tenantModel: payload.blueprint.selections.saas.some(
          ({ id, lifecycle }) =>
            id === "saas.team-organizations" && lifecycle.selected,
        )
          ? "organization"
          : "personal",
        chargingModel: payload.blueprint.selections.saas.some(
          ({ id, lifecycle }) =>
            id.startsWith("saas.billing-") && lifecycle.selected,
        )
          ? "subscription-user"
          : "free",
      },
    ...
    try {
      ...
      setCfpgCommands({
        development:
          result.blueprint.providers.database.cfpg?.development
            ?.connectCommand || "",
        production:
          result.blueprint.providers.database.cfpg?.production
            ?.connectCommand || "",
      });
      setDirty(false);
      setSaveState("saved");
  ...
  const generateProject = async () => {
    ...
    try {
      const response = await fetch("/__starter/generate", {
        ...
        body: JSON.stringify({
          name: payload.blueprint.project.name,
          slug: payload.blueprint.project.slug,
        }),
      });
      const result = (await response.json()) as {
        error?: string;
      ...
      setGeneration({
        ...
        message: result.archive
          ? `Independent project and package generated: ${result.archive}`
          : "Independent project generated.",
    ...
    <div className="setup-shell" lang="en" translate="no">
      <header className="setup-header">
        ...
        <div className="setup-header-actions">
          <span
            className={`setup-save-state ${dirty ? "unsaved" : saveState}`}
            role="status"
          >
            {saveState === "saving"
              ? "Saving…"
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            updateBlueprint((blueprint) => {
                              ...
                              const nativeMobile = platforms.some(
                                (item) => item === "ios" || item === "android",
                              );
                              return {
                                ...blueprint,
                                ...
                                selections: {
                                  ...blueprint.selections,
                                  capabilities:
                                    blueprint.selections.capabilities.map(
                                      (selection) =>
                                        selection.id === "capability.expo-push"
                                          ? {
                                              ...selection,
          ...
          {currentStep.id === "saas" ? (
            <div className="setup-stack">
              <section className="setup-panel">
                <div className="panel-title">
                  <div>
                    <h2>Core product foundation</h2>
                    <p>
                      Permanent infrastructure for the selected product shape is
                      visible here and does not need to be selected again.
                    </p>
                  </div>
                ...
                <div className="pack-grid core-capability-grid">
                  {[
                    ...
                    <article
                      className="pack-choice selected core-choice"
                      key={name}
                    >
                      <span className="pack-choice-main">
                        <span>
                        ...
                        <p>
                          Included in the product foundation and registered in
                          the project Agent Map.
                        </p>
                      </span>
                      <span className="pack-check">
                        <Check size={15} />
              ...
              <p className="setup-field-help">
                ...
                {payload.saasCapabilities.capabilities.length} named product
                modules. Planned modules remain visible in /dp, not selectable
                here.
              </p>
              ...
              <div className="panel-title">
                <div>
                  <h2>Optional product modules</h2>
                  <p>
                    Add only behavior the selected Web, content or mobile
              ...
              <div className="pack-grid">
                <label
                  className={
                    billingSelected ? "pack-choice selected" : "pack-choice"
                  }
                >
                  <input
                    type="checkbox"
                    ...
                    onChange={(event) =>
                      setBillingCapability(event.target.checked)
                    }
                  />
                  <span className="pack-choice-main">
                    <span>
                    ...
                    <p>
                      Checkout, Portal and subscription lifecycle. Choose
                      Stripe, Polar or Autumn in Providers.
                    </p>
                  </span>
                  <span className="pack-check">
                    <Check size={15} />
                ...
                {packsFor("saas")
                  .filter(
                    (pack) =>
                      !pack.id.startsWith("saas.billing-") &&
                      !advancedIdentityPackIds.has(pack.id),
                  )
                  .map((pack) => (
                    <PackChoice
                      key={pack.id}
              ...
              <details className="setup-panel advanced-identity-section">
                <summary>
                  <span>
                    <strong>Advanced Identity & Access</strong>
                    <small>
                      Better Auth plugins · loaded only when selected
                    </small>
                  </span>
                </summary>
                <p>
                  Passkeys, enterprise identity, API/Agent authorization and
                  optional login methods remain independent Packs. Selecting
                  none adds no code, dependency, route or database migration.
                </p>
                <div className="pack-grid">
                  {packsFor("saas")
                    .filter((pack) => advancedIdentityPackIds.has(pack.id))
                    .map((pack) => (
                      <PackChoice
                        key={pack.id}
                        pack={pack}
            ...
            <div className="setup-stack provider-setup">
              <section className="setup-panel provider-summary">
                <div>
                  ...
                  <small>
                    Identity, sessions and selected sign-in Providers
                  </small>
                </div>
                <div>
                  <span>Database</span>
                  <strong>Native PostgreSQL</strong>
                  <small>
                    Isolated Hyperdrive /{" "}
                ...
                <div>
                  ...
                  <small>
                    Activated only when the Billing pack is materialized
                  </small>
                </div>
              </section>
              <nav className="provider-tabs" aria-label="Provider categories">
                {(
                  ...
                  <button
                    type="button"
                    key={id}
                    className={providerTab === id ? "active" : ""}
              ...
              <section
                className="setup-panel provider-section"
                hidden={providerTab !== "advanced"}
              >
                <header>
                  <h2>Maps and geocoding</h2>
                  <p>
                    Selecting None generates no map code or dependency. A
                    Provider becomes selectable only after its adapter,
                    credentials, Setup links, verification and removal contract
                    are implemented.
                  </p>
                </header>
                <div
                  className="provider-option-grid"
                  role="radiogroup"
                  aria-label="Maps Provider"
                >
                  {mapsCategory.options
                    .filter(({ id }) => id !== "maptiler")
                    .map((option) => {
                      const selected = selectedMapProvider === option.id;
                      return (
                        <button
                          type="button"
                          className={
                          ...
                          onClick={() =>
                            setPackSelected(mapPack, option.id === "mapcn")
                          }
                          key={option.id}
                        >
                          <span className="pack-check">
                          ...
                          <span>
                            ...
                            <small>
                              {option.selectable
                                ? option.notes
                                : "Planned · adapter not yet available"}
                            </small>
                          </span>
                        </button>
                      );
              ...
              <section
                className="setup-panel provider-section"
                hidden={providerTab !== "essentials"}
              >
                <header>
                  <h2>PostgreSQL</h2>
                  <p>
                    Application code uses the native PostgreSQL contract through
                    isolated Development and Production Hyperdrive bindings.
                    Choose SQL-first or Drizzle only for product-domain code.
                  </p>
                </header>
                <div
                  className="storage-provider-options"
                  role="group"
                  aria-label="Product data layer"
                >
                  {(["sql-first", "drizzle"] as const).map((access) => (
                    <button
                      type="button"
                      key={access}
                      aria-pressed={
                        payload.blueprint.providers.database.access === access
                      }
                      onClick={() =>
                        updateBlueprint((blueprint) => ({
                          ...blueprint,
                          ...
                          selections: {
                            ...blueprint.selections,
                            capabilities: blueprint.selections.capabilities.map(
                              (selection) =>
                                selection.id === "capability.data-layer-drizzle"
                                  ? {
                                      ...selection,
                                      lifecycle: {
                                        ...selection.lifecycle,
                    ...
                    >
                      <strong>
                        {access === "sql-first" ? "SQL" : "Drizzle"}
                      </strong>
                      <span>
                        {access === "sql-first"
                          ? "Smallest and AI-first."
                          : "Typed product-domain schema over the same pg connection."}
                      </span>
                    </button>
                  ))}
                </div>
                <div className="provider-default-card">
                  <div>
                    <strong>Native PostgreSQL + Hyperdrive</strong>
                    <small>
                      Development and Production use independent databases and
                      Hyperdrive bindings. Setup does not offer an alternate
                      database transport.
                    </small>
                  </div>
                </div>
              ...
              >
                <header>
                  ...
                  <p>
                    Keep file bytes outside PostgreSQL. None adds no runtime; R2
                    uses a native Worker Binding and local simulation;
                    S3-compatible adds its SDK only when selected.
                  </p>
                </header>
                <div
                  className="provider-option-grid"
                  role="radiogroup"
                  aria-label="Object storage Provider"
                >
                  {[
                    {
                      id: "none",
                      name: "None",
                      note: "No upload API, storage dependency, bucket or Binding.",
                    },
                    {
                      id: "cloudflare-r2",
                      name: "Cloudflare R2",
                      note: "Recommended: local R2 simulation plus isolated cloud buckets.",
                    },
                    {
                      id: "s3-compatible",
                      name: "S3-compatible",
                      note: "AWS S3, MinIO, R2 S3 API, Backblaze or Wasabi.",
                    },
                  ].map(({ id, name, note }) => {
                    const selected =
                      payload.blueprint.providers.storage.provider === id;
                    return (
                      <div
                        className={
                          selected
                  ...
                  <div className="storage-provider-config">
                    <div className="setup-fields">
                      <label className="setup-field">
                        ...
                        <select
                          value={payload.blueprint.providers.storage.access}
                          onChange={(event) =>
                            updateBlueprint((blueprint) => ({
                              ...blueprint,
                              providers: {
                        ...
                        >
                          ...
                          <option value="public">
                            Public through authorized Worker route
                          </option>
                        </select>
                      </label>
                      <label className="setup-field">
                        ...
                        <select
                          value={
                            payload.blueprint.providers.storage.maxUploadBytes
                          }
                          onChange={(event) =>
                            updateBlueprint((blueprint) => ({
                              ...blueprint,
                        ...
                        >
                          ...
                          <option value={5_242_880}>5 MiB</option>
                          <option value={10_485_760}>10 MiB</option>
                        </select>
                      </label>
                    </div>
                    <div className="storage-environment-grid">
                      {(["development", "production"] as const).map(
                        (environment) => {
                          const value =
                            payload.blueprint.providers.storage[environment];
                          const update = (patch: Partial<StorageEnvironment>) =>
                            updateBlueprint((blueprint) => ({
                              ...blueprint,
                              providers: {
                                ...blueprint.providers,
                                storage: {
                                  ...blueprint.providers.storage,
                                  [environment]: {
                                    ...blueprint.providers.storage[environment],
                                    ...patch,
                                  },
                                },
                              },
                          ...
                          return (
                            <div key={environment}>
                              ...
                              <Field
                                ...
                                value={value.publicDomain}
                                onChange={(publicDomain) =>
                                  update({ publicDomain })
                                }
                              />
                              {payload.blueprint.providers.storage.provider ===
                              "s3-compatible" ? (
                                <>
                                  <Field
                                    label="S3 endpoint"
                                  ...
                                  <label className="platform-option">
                                    <input
                                      type="checkbox"
                                      checked={value.s3ForcePathStyle}
                                      onChange={(event) =>
                                        update({
                                          s3ForcePathStyle:
                                ...
                                <p>
                                  Local Worker development uses an empty
                                  simulated R2 bucket; cloud provisioning
                                  creates this environment's real bucket.
                                </p>
                              )}
                            </div>
                          );
                    ...
                    "s3-compatible" ? (
                      <ProviderCredentialEditor
                        ...
                        onEditing={(editing) =>
                          setProviderEditing("s3-compatible", editing)
                        }
                        onChange={updateProviderSecret}
                      />
                    ) : (
                      <div className="provider-resource-links">
                        <a
              ...
              >
                <header>
                  <h2>Anti-abuse</h2>
                  <p>
                    Turnstile protects credential registration, sign-in and
                    password reset. The browser widget and server-side
                    Siteverify are both mandatory.
                  </p>
                </header>
                <div
                  className="provider-option-grid"
                  role="radiogroup"
                  aria-label="Anti-abuse Provider"
                >
                  {[
                    {
                      id: "none",
                      name: "None",
                      note: "No challenge Provider. Authentication rate limits still apply.",
                    },
                    {
                      id: "turnstile",
                      name: "Cloudflare Turnstile",
                      note: "Recommended for public credential flows; no puzzle for most legitimate users.",
                    },
                  ].map(({ id, name, note }) => {
                    const selected =
                      payload.blueprint.providers.antiAbuse.provider === id;
                    return (
                      <div
                        className={
                          selected
                      ...
                      >
                        <label>
                          <input
                            type="radio"
                            name="anti-abuse-provider"
                            checked={selected}
                            onChange={() =>
                              setAntiAbuseProvider(
                                id as AntiAbusePolicy["provider"],
                              )
                            }
                          />
                          <span>
                ...
                {payload.blueprint.providers.antiAbuse.provider ===
                "turnstile" ? (
                  <div className="storage-provider-config">
                    <div className="storage-environment-grid">
                      {(["development", "production"] as const).map(
                        (environment) => (
                          <div key={environment}>
                            ...
                            <Field
                              ...
                              value={
                                payload.blueprint.providers.antiAbuse[
                                  environment
                                ].siteKey
                              }
                              onChange={(siteKey) =>
                            ...
                            />
                            <p>
                              Use a different widget and hostname allowlist for
                              this environment.
                            </p>
                          </div>
                        ),
                    ...
                    <ProviderCredentialEditor
                      provider="turnstile"
                      state={payload.providerCredentials.turnstile}
                      editing={providerEditors.turnstile}
                      values={providerSecrets}
                      onEditing={(editing) =>
                        setProviderEditing("turnstile", editing)
                      }
                      onChange={updateProviderSecret}
                    />
                    <div className="provider-resource-links">
                      {providerSetupLinks.turnstile.map((link) => (
                        <a
                          href={link.href}
                          target="_blank"
                          rel="noreferrer"
                    ...
                    {payload.blueprint.providers.antiAbuse.development
                      .siteKey ? (
                      <div className="provider-email-test">
                        <div>
                          <strong>Real Siteverify test</strong>
                          <p>
                            Complete the Development widget, then validate its
                            one-time token with the saved or newly entered
                            Development secret.
                          </p>
                        </div>
                        <TurnstileChallenge
                          ...
                          action="starter_setup_test"
                          onToken={(token) => {
                            setTurnstileToken(token);
                            if (token) setTurnstileTest({ status: "idle" });
                          }}
                          onError={(message) =>
                            setTurnstileTest({
                              status: "error",
                        ...
                        <Button
                          ...
                          disabled={
                            !turnstileToken ||
                            turnstileTest.status === "testing"
                          }
                          onClick={() => void runTurnstileTest()}
                        >
                          {turnstileTest.status === "testing"
                            ? "Validating challenge"
                            : "Validate Turnstile"}
                        </Button>
                        {turnstileTest.message ? (
                          <p
                            className={
                              turnstileTest.status === "success"
                      ...
                      <p className="provider-test-unavailable">
                        Enter the Development site key to load the validation
                        widget.
                      </p>
                    )}
                    <div className="provider-live-test">
                      <Button asChild type="button" size="sm" variant="outline">
                        <a
                          href={`https://${payload.config.development.domain}/login`}
                          target="_blank"
                          rel="noreferrer"
                        >
                          Test protected auth on Development
                          <ExternalLink size={13} />
                        </a>
                      </Button>
                      <small>
                        After a Development release, this opens the real
                        sign-up, sign-in and password-reset enforcement path.
                      </small>
                    </div>
                  </div>
                ) : null}
              ...
              >
                <header>
                  <h2>AI models and Gateway</h2>
                  <p>
                    Workers AI keeps inference inside the Cloudflare runtime. AI
                    Gateway is an optional per-environment overlay for logs,
                    caching, routing and billing; no client API key is exposed.
                  </p>
                </header>
                <div
                  className="provider-option-grid"
                  ...
                  {[
                    {
                      id: "none",
                      name: "None",
                      note: "No AI Binding, model traffic or AI runtime code.",
                    },
                    {
                      id: "workers-ai",
                      name: "Cloudflare Workers AI",
                      note: "Native AI Binding with an optional AI Gateway ID.",
                    },
                  ].map(({ id, name, note }) => {
                    const selected =
                      payload.blueprint.providers.ai.provider === id;
                    return (
                      <div
                        className={
                          selected
                      ...
                      >
                        <label>
                          <input
                            ...
                            checked={selected}
                            onChange={() =>
                              setAiProvider(id as AiPolicy["provider"])
                            }
                          />
                          <span>
                            <strong>{name}</strong>
                ...
                {payload.blueprint.providers.ai.provider === "workers-ai" ? (
                  <div className="storage-provider-config">
                    <div className="storage-environment-grid">
                      {(["development", "production"] as const).map(
                        (environment) => {
                          const value =
                            payload.blueprint.providers.ai[environment];
                          const update = (
                            patch: Partial<AiPolicy[typeof environment]>,
                          ) =>
                            updateBlueprint((blueprint) => ({
                              ...blueprint,
                              providers: {
                                ...
                                ai: {
                                  ...blueprint.providers.ai,
                                  [environment]: {
                                    ...blueprint.providers.ai[environment],
                                    ...patch,
                                  },
                                },
                              },
                          ...
                          return (
                            <div key={environment}>
                              ...
                              <Field
                                ...
                                onChange={(gatewayId) => update({ gatewayId })}
                              />
                              <p>
                                Leave Gateway empty for a direct Binding call.
                                Use an existing Gateway ID or{" "}
                                <code>default</code> to let Cloudflare
                                create/use the account default on the first
                    ...
                    <div className="provider-resource-links">
                      {providerSetupLinks["workers-ai"].map((link) => (
                        <a
                          href={link.href}
                          target="_blank"
                          rel="noreferrer"
                    ...
                    <div className="provider-email-test">
                      <div>
                        <strong>Real Workers AI test</strong>
                        <p>
                          Uses the saved Cloudflare account token to call the
                          selected Development model. The prompt is fixed and
                          output is bounded.
                        </p>
                      </div>
                      <Button
                        ...
                        variant="outline"
                        disabled={
                          workersAiTest.status === "testing" ||
                          !payload.blueprint.providers.ai.development.model
                        }
                        onClick={() => void runWorkersAiTest()}
                      >
                        {workersAiTest.status === "testing"
                          ? "Running Workers AI"
                          : "Test Development model"}
                      </Button>
                      {workersAiTest.message ? (
                        <p
                          className={
                            workersAiTest.status === "success"
                              ? "provider-test-result success"
                              : "provider-test-result error"
                          }
                          role="status"
                    ...
                    <div className="provider-live-test">
                      <Button asChild type="button" size="sm" variant="outline">
                        <a
                          href={`https://${payload.config.development.domain}/admin`}
                          target="_blank"
                          rel="noreferrer"
                        >
                          Test deployed AI Binding
                          <ExternalLink size={13} />
                        </a>
                      </Button>
                      <small>
                        The local REST test proves provider access; Development
                        acceptance still requires the deployed Worker Binding
                        and Admin test route.
                      </small>
                    </div>
                  </div>
                ) : null}
              ...
              >
                <header>
                  <h2>Product search and vector index</h2>
                  <p>
                    Use PostgreSQL first for ordinary product search. Select
                    Vectorize only for embedding similarity or RAG; its
                    dimensions and distance metric are immutable after index
                    creation.
                  </p>
                </header>
                <div
                  ...
                  {[
                    {
                      id: "none",
                      name: "None",
                      note: "No product-data search index. Registered route search remains available.",
                    },
                    {
                      id: "postgresql",
                      name: "PostgreSQL search",
                      note: "Recommended for text/filter search without another Cloudflare resource.",
                    },
                    {
                      id: "vectorize",
                      name: "Cloudflare Vectorize",
                      note: "Embedding similarity and RAG with isolated environment indexes.",
                    },
                  ].map(({ id, name, note }) => {
                    const selected =
                      payload.blueprint.providers.search.provider === id;
                    return (
                      <div
                        className={
                          selected
                      ...
                      >
                        <label>
                          <input
                            ...
                            checked={selected}
                            onChange={() =>
                              setSearchProvider(id as SearchPolicy["provider"])
                            }
                          />
                          <span>
                            <strong>{name}</strong>
                  ...
                  <div className="storage-provider-config">
                    <div className="storage-environment-grid">
                      {(["development", "production"] as const).map(
                        (environment) => {
                          const value =
                            payload.blueprint.providers.search[environment];
                          const update = (
                            patch: Partial<SearchPolicy[typeof environment]>,
                          ) =>
                            updateBlueprint((blueprint) => ({
                              ...blueprint,
                              providers: {
                                ...
                                search: {
                                  ...blueprint.providers.search,
                                  [environment]: {
                                    ...blueprint.providers.search[environment],
                                    ...patch,
                                  },
                                },
                              },
                              ...
                              <label className="setup-field">
                                ...
                                <select
                                  ...
                                  onChange={(event) =>
                                    update({
                                      metric: event.target
                                        .value as SearchPolicy[typeof environment]["metric"],
                                    })
                                  }
                                >
                                  <option value="cosine">Cosine</option>
                                  <option value="euclidean">Euclidean</option>
                                  <option value="dot-product">
                                    Dot product
                                  </option>
                                </select>
                              </label>
                              <p>
                                Dimensions must match the chosen embedding model
                                and cannot be changed on an existing index.
                              </p>
                            </div>
                          );
                        },
                    ...
                    <div className="provider-resource-links">
                      {providerSetupLinks.vectorize.map((link) => (
                        <a
                          href={link.href}
                          target="_blank"
                          rel="noreferrer"
                    ...
                    <div className="provider-email-test">
                      <div>
                        <strong>Real Vectorize round trip</strong>
                        <p>
                          Requires the Development index to be provisioned. The
                          test inserts, queries and deletes one generated vector
                          without exposing product data.
                        </p>
                      </div>
                      <Button
                        type="button"
                        variant="outline"
                        disabled={vectorizeTest.status === "testing"}
                        onClick={() => void runVectorizeTest()}
                      >
                        {vectorizeTest.status === "testing"
                          ? "Testing Vectorize"
                          : "Test Development index"}
                      </Button>
                      {vectorizeTest.message ? (
                        <p
                          className={
                            vectorizeTest.status === "success"
                              ? "provider-test-result success"
                              : "provider-test-result error"
                          }
                          role="status"
              ...
              <section
                className="setup-panel provider-section"
                hidden={providerTab !== "advanced" || !nativeMobileSelected}
              >
                <header>
                  <h2>Native push notifications</h2>
                  <p>
                    Expo Push is only for native iOS/Android builds. It adds
                    notification permissions, Android channel setup, user-owned
                    device tokens and server delivery; Expo Go is not a physical
                    delivery acceptance target.
                  </p>
                </header>
                <div
                  className={
                ...
                >
                  ...
                  <div>
                    <strong>Expo Push</strong>
                    <small>
                      {payload.blueprint.providers.push.provider === "expo-push"
                        ? "Mobile default · included automatically for iOS and Android."
                        : "Removed from this generated project."}
                    </small>
                  </div>
                  <Button
                    type="button"
                    ...
                    onClick={() =>
                      setPushProvider(
                        payload.blueprint.providers.push.provider ===
                          "expo-push"
                          ? "none"
                          : "expo-push",
                      )
                ...
                {payload.blueprint.providers.push.provider === "expo-push" ? (
                  <div className="storage-provider-config">
                      ...
                      {(["development", "production"] as const).map(
                        (environment) => (
                          <div key={environment}>
                            <h3>
                              {environment === "development"
                                ? "Development Expo project"
                                : "Production Expo project"}
                            </h3>
                            <Field
                              label="EAS project ID"
                              value={
                                payload.blueprint.providers.push[environment]
                                  .projectId
                              }
                              onChange={(projectId) =>
                                updateBlueprint((blueprint) => ({
                            ...
                            <p>
                              ...
                              environment so device tokens and credentials
                              cannot cross release lanes.
                            </p>
                          </div>
                        ),
                      )}
                        ...
                        onChange={(event) =>
                          updateBlueprint((blueprint) => ({
                            ...
                            providers: {
                              ...
                              push: {
                                ...blueprint.providers.push,
                                accessTokenRequired: event.target.checked,
                              },
                            },
                          }))
                        }
                      />
                      Require Expo Push access-token security
                    </label>
                    {payload.blueprint.providers.push.accessTokenRequired ? (
                      <ProviderCredentialEditor
                        provider="expo-push"
                        state={payload.providerCredentials["expo-push"]}
                        editing={providerEditors["expo-push"]}
                        values={providerSecrets}
                        onEditing={(editing) =>
                          setProviderEditing("expo-push", editing)
                        }
                        onChange={updateProviderSecret}
                      />
                    ) : null}
                    <div className="provider-resource-links">
                      {providerSetupLinks["expo-push"].map((link) => (
                        <a
                          href={link.href}
                          target="_blank"
                          rel="noreferrer"
                    ...
                    <div className="provider-email-test">
                      <div>
                        <strong>Real Development push test</strong>
                        <p>
                          Paste an ExpoPushToken from a signed-in physical
                          Development build. Setup sends a fixed message and
                          returns only the Expo ticket ID.
                        </p>
                      </div>
                      <label>
                        <span>Development ExpoPushToken</span>
                        <Input
                          value={expoPushToken}
                          autoComplete="off"
                          onChange={(event) => {
                            setExpoPushToken(event.target.value);
                            setExpoPushTest({ status: "idle" });
                          }}
                      ...
                      <Button
                        ...
                        variant="outline"
                        disabled={
                          expoPushTest.status === "testing" ||
                          !expoPushToken.trim()
                        }
                        onClick={() => void runExpoPushTest()}
                      >
                        {expoPushTest.status === "testing"
                          ? "Sending push"
                          : "Send Development push"}
                      </Button>
                      {expoPushTest.message ? (
                        <p
                          className={
                            expoPushTest.status === "success"
                              ? "provider-test-result success"
                              : "provider-test-result error"
                          }
                          role="status"
              ...
              >
                <header>
                  ...
                  <p>
                    SMS is an explicit server channel, not a default second
                    factor and not implied by in-app notifications. Twilio
                    credentials, sender numbers, compliance and delivery
                    evidence remain environment-specific.
                  </p>
                </header>
                <div
                  className="provider-option-grid"
                  ...
                  {[
                    {
                      id: "none",
                      name: "None",
                      note: "No SMS provider, secret requirements or delivery ledger.",
                    },
                    {
                      id: "twilio",
                      name: "Twilio SMS",
                      note: "Programmable Messaging with API-key authentication and SQL idempotency.",
                    },
                  ].map(({ id, name, note }) => {
                    const selected =
                      payload.blueprint.providers.sms.provider === id;
                    return (
                      <div
                        className={
                          selected
                      ...
                      >
                        <label>
                          <input
                            ...
                            checked={selected}
                            onChange={() =>
                              setSmsProvider(id as SmsPolicy["provider"])
                            }
                          />
                          <span>
                            <strong>{name}</strong>
                ...
                {payload.blueprint.providers.sms.provider === "twilio" ? (
                  <div className="storage-provider-config">
                      ...
                      {(["development", "production"] as const).map(
                        (environment) => (
                          <div key={environment}>
                            <h3>
                              {environment === "development"
                                ? "Development Twilio region"
                                : "Production Twilio region"}
                            </h3>
                            <Field
                              label="API base URL"
                              value={
                                payload.blueprint.providers.sms[environment]
                                  .apiBaseUrl
                              }
                              onChange={(apiBaseUrl) =>
                                updateBlueprint((blueprint) => ({
                            ...
                            />
                            <p>
                              Use <code>https://api.twilio.com</code> or the
                              documented regional base URL. Setup requires
                              HTTPS.
                            </p>
                          </div>
                        ),
                    ...
                    <ProviderCredentialEditor
                      provider="twilio-sms"
                      state={payload.providerCredentials["twilio-sms"]}
                      editing={providerEditors["twilio-sms"]}
                      values={providerSecrets}
                      onEditing={(editing) =>
                        setProviderEditing("twilio-sms", editing)
                      }
                      onChange={updateProviderSecret}
                    />
                    <div className="provider-resource-links">
                      {providerSetupLinks["twilio-sms"].map((link) => (
                        <a
                          href={link.href}
                          target="_blank"
                          rel="noreferrer"
                    ...
                    <div className="provider-email-test">
                      <div>
                        <strong>Real Development SMS test</strong>
                        <p>
                          Twilio receives a fixed message. Setup returns only
                          the provider SID, initial status and recipient last
                          four digits.
                        </p>
                      </div>
                      <label>
                        <span>Test recipient (E.164)</span>
                        <Input
                          type="tel"
                          value={smsRecipient}
                          autoComplete="tel"
                      ...
                      <Button
                        ...
                        variant="outline"
                        disabled={
                          smsTest.status === "testing" || !smsRecipient.trim()
                        }
                        onClick={() => void runSmsTest()}
                      >
                        {smsTest.status === "testing"
                          ? "Sending SMS"
                          : "Send Development SMS"}
                      </Button>
                      {smsTest.message ? (
                        <p
                          className={
                            smsTest.status === "success"
                              ? "provider-test-result success"
                              : "provider-test-result error"
                          }
                          role="status"
              ...
              >
                <header>
                  ...
                  <p>
                    Cloudflare Images transforms raw image bytes inside the
                    Worker. R2 or S3 remains the original-file authority; this
                    Pack adds resize/transcode behavior only.
                  </p>
                </header>
                <div
                  className="provider-option-grid"
                  ...
                  {[
                    {
                      id: "none",
                      name: "None",
                      note: "Ordinary files still use Object Storage; no image transformation Binding.",
                    },
                    {
                      id: "cloudflare-images",
                      name: "Cloudflare Images",
                      note: "Worker-native image info, resize and modern-format output.",
                    },
                  ].map(({ id, name, note }) => {
                    const selected =
                      payload.blueprint.providers.media.images.provider === id;
                    return (
                      <div
                        className={
                          selected
                      ...
                      >
                        <label>
                          <input
                            ...
                            onChange={() =>
                              setImagesProvider(
                                id as MediaPolicy["images"]["provider"],
                              )
                            }
                          />
                          <span>
                ...
                {payload.blueprint.providers.media.images.provider ===
                "cloudflare-images" ? (
                  <div className="storage-provider-config">
                    <div className="setup-fields">
                      <label className="setup-field">
                        ...
                        <select
                          value={
                            payload.blueprint.providers.media.images
                              .maxInputBytes
                          }
                          onChange={(event) =>
                            updateBlueprint((blueprint) => ({
                        ...
                        >
                          <option value={5_242_880}>5 MiB</option>
                          <option value={10_485_760}>10 MiB</option>
                          <option value={20_971_520}>
                            20 MiB platform maximum
                          </option>
                        </select>
                      </label>
                      <label className="setup-field">
                                ...
                                media: {
                                  ...
                                  images: {
                                    ...
                                    defaultFormat: event.target
                                      .value as MediaPolicy["images"]["defaultFormat"],
                                  },
                                },
                              },
                            }))
                    ...
                    <div className="provider-resource-links">
                      {providerSetupLinks["cloudflare-images"].map((link) => (
                        <a
                          href={link.href}
                          target="_blank"
                          rel="noreferrer"
                    ...
                    <div className="provider-live-test">
                      <Button asChild type="button" size="sm" variant="outline">
                        <a
                          ...
                          rel="noreferrer"
                        >
                          Test Images Binding on Development
                          <ExternalLink size={13} />
                        </a>
                      </Button>
                      <small>
                        The Pack exposes an Admin-only fixed 2×2 PNG → 1×1 WebP
                        round trip. Local Workerd uses Cloudflare's low-fidelity
                        binding; Development verifies the high-fidelity Binding.
                      </small>
                    </div>
                  </div>
                ) : null}
              ...
              >
                <header>
                  ...
                  <p>
                    Cloudflare Stream creates one-time direct upload URLs so
                    clients never receive the API token. This initial Pack
                    supports public playback; private signed playback remains
                    unavailable until signing-token generation is implemented.
                  </p>
                </header>
                <div
                  className="provider-option-grid"
                  ...
                  {[
                    {
                      id: "none",
                      name: "None",
                      note: "No video upload, encoding, webhook or playback runtime.",
                    },
                    {
                      id: "cloudflare-stream",
                      name: "Cloudflare Stream",
                      note: "User-owned direct uploads, encoding state and public HLS/DASH playback.",
                    },
                  ].map(({ id, name, note }) => {
                    const selected =
                      payload.blueprint.providers.media.stream.provider === id;
                    return (
                      <div
                        className={
                          selected
                ...
                "cloudflare-stream" ? (
                  <div className="storage-provider-config">
                    <label className="setup-field">
                      <span>Maximum video duration</span>
                      <select
                        value={
                          payload.blueprint.providers.media.stream
                            .maxDurationSeconds
                        }
                        onChange={(event) =>
                          updateBlueprint((blueprint) => ({
                          ...
                          return (
                            <div key={environment}>
                              ...
                              <Field
                                ...
                                onChange={(origins) =>
                                  update({ allowedOrigins: parseList(origins) })
                                }
                              />
                              <Field
                                label="API base URL"
                    ...
                    <ProviderCredentialEditor
                      provider="cloudflare-stream"
                      state={payload.providerCredentials["cloudflare-stream"]}
                      editing={providerEditors["cloudflare-stream"]}
                      values={providerSecrets}
                      onEditing={(editing) =>
                        setProviderEditing("cloudflare-stream", editing)
                      }
                      onChange={updateProviderSecret}
                    />
                    <div className="provider-resource-links">
                      {providerSetupLinks["cloudflare-stream"].map((link) => (
                        <a
                          href={link.href}
                          target="_blank"
                          rel="noreferrer"
                    ...
                    <div className="provider-email-test">
                      <div>
                        <strong>Real Stream token test</strong>
                        <p>
                          Creates a one-time one-second direct-upload draft and
                          immediately deletes it without uploading media.
                        </p>
                      </div>
                      <Button
                        type="button"
                        variant="outline"
                        disabled={streamTest.status === "testing"}
                        onClick={() => void runStreamTest()}
                      >
                        {streamTest.status === "testing"
                          ? "Testing Stream"
              ...
              >
                <header>
                  ...
                  <p>
                    Cron Triggers run in UTC and should start only bounded,
                    idempotent product jobs. The Starter Pack records a
                    heartbeat; copied products register their own work
                    explicitly.
                  </p>
                </header>
                <div
                  className="provider-option-grid"
                  role="radiogroup"
                  aria-label="Cron Provider"
                >
                  {[
                    {
                      id: "none",
                      name: "None",
                      note: "No scheduled handler or deployed Cron Trigger.",
                    },
                    {
                      id: "cron",
                      name: "Cloudflare Cron",
                      note: "Environment-specific schedule with SQL run evidence.",
                    },
                  ].map(({ id, name, note }) => {
                    const selected =
                      payload.blueprint.providers.background.cron.enabled ===
                      (id === "cron");
                    return (
                      <div
                        className={
                      ...
                      >
                        <label>
                          <input
                            ...
                            name="cron-provider"
                            checked={selected}
                            onChange={() => setCronEnabled(id === "cron")}
                          />
                          <span>
                            <strong>{name}</strong>
                            <small>{note}</small>
                      ...
                      {(["development", "production"] as const).map(
                        (environment) => (
                          <div key={environment}>
                            ...
                            <Field
                              label="UTC Cron expression"
                              value={
                                payload.blueprint.providers.background.cron[
                                  environment
                                ].expression
                              }
                              onChange={(expression) =>
                            ...
                            />
                            <p>
                              The expression is copied exactly to Wrangler and
                              is available as <code>controller.cron</code>.
                            </p>
                          </div>
                        ),
                      )}
                    ...
                    <div className="provider-live-test">
                      <Button asChild type="button" size="sm" variant="outline">
                        <a
                          ...
                          rel="noreferrer"
                        >
                          View Development Cron evidence
                          <ExternalLink size={13} />
                        </a>
                      </Button>
                      <small>
                        Local verification invokes Wrangler's scheduled test
                        route and reads the heartbeat; Development release
                        verifies the deployed trigger after propagation.
                      </small>
                    </div>
                  </div>
                ) : null}
              ...
              >
                <header>
                  <h2>Durable multi-step jobs</h2>
                  <p>
                    Cloudflare Workflows persists step results and retries
                    across Worker invocations. Starter installs only a fixed
                    two-step test skeleton; copied products replace its payload
                    and steps.
                  </p>
                </header>
                <div
                  className="provider-option-grid"
                  role="radiogroup"
                  aria-label="Workflow Provider"
                >
                  {[
                    {
                      id: "none",
                      name: "None",
                      note: "No Workflow class, binding, instance API or remote resource.",
                    },
                    {
                      id: "workflows",
                      name: "Cloudflare Workflows",
                      note: "Durable step execution with Admin instance create/status.",
                    },
                  ].map(({ id, name, note }) => {
                    const selected =
                      payload.blueprint.providers.background.workflow
                        .enabled ===
                      (id === "workflows");
                    return (
                      <div
                      ...
                      >
                        <label>
                          <input
                            ...
                            checked={selected}
                            onChange={() =>
                              setWorkflowsEnabled(id === "workflows")
                            }
                          />
                          <span>
                            <strong>{name}</strong>
                ...
                {payload.blueprint.providers.background.workflow.enabled ? (
                  <div className="storage-provider-config">
                    <label className="platform-option">
                      <input
                        type="checkbox"
                      ...
                      />
                      Create Workflow instances on a schedule
                    </label>
                    {payload.blueprint.providers.background.workflow
                      .scheduleEnabled ? (
                      <div className="storage-environment-grid">
                        {(["development", "production"] as const).map(
                          (environment) => (
                            <div key={environment}>
                              <h3>
                                {environment === "development"
                                  ? "Development Workflow schedule"
                                  : "Production Workflow schedule"}
                              </h3>
                              <Field
                                label="UTC Cron expression"
                                value={
                                  payload.blueprint.providers.background
                                    .workflow[environment].expression
                                }
                                onChange={(expression) =>
                                  updateBlueprint((blueprint) => ({
                                    ...blueprint,
                    ...
                    <div className="provider-live-test">
                      <Button asChild type="button" size="sm" variant="outline">
                        <a
                          ...
                          rel="noreferrer"
                        >
                          Test Workflow on Development
                          <ExternalLink size={13} />
                        </a>
                      </Button>
                      <small>
                        The Admin test creates a fixed instance and reads
                        status/output. A deselected release removes the
                        binding/class first, then deletes the recorded remote
                        Workflow resource.
                      </small>
                    </div>
                  </div>
              ...
              >
                <header>
                  ...
                  <p>
                    Durable Objects keep one strongly consistent SQLite-backed
                    state authority per room. Hibernatable WebSockets preserve
                    client connections while idle without keeping the object
                    billed as active.
                  </p>
                </header>
                <div
                  className="provider-option-grid"
                  role="radiogroup"
                  aria-label="Realtime Provider"
                >
                  {[
                    {
                      id: "none",
                      name: "None",
                      note: "No Durable Object class, Binding, room route or WebSocket runtime.",
                    },
                    {
                      id: "durable-objects",
                      name: "Durable Objects / WebSockets",
                      note: "Authenticated per-room messages, sequence state and hibernatable sockets.",
                    },
                  ].map(({ id, name, note }) => {
                    const selected =
                      payload.blueprint.providers.background.realtime
                        .enabled ===
                      (id === "durable-objects");
                    return (
                      <div
                      ...
                      >
                        <label>
                          <input
                            ...
                            checked={selected}
                            onChange={() =>
                              setRealtimeEnabled(id === "durable-objects")
                            }
                          />
                          <span>
                            <strong>{name}</strong>
                ...
                {payload.blueprint.providers.background.realtime.enabled ? (
                  <div className="storage-provider-config">
                    <div className="provider-resource-links">
                      <a
                    ...
                    <div className="provider-live-test">
                      <Button asChild type="button" size="sm" variant="outline">
                        <a
                          ...
                          rel="noreferrer"
                        >
                          Test realtime room on Development
                          <ExternalLink size={13} />
                        </a>
                      </Button>
                      <small>
                        The Admin test opens the real Durable Object socket,
                        sends one bounded message, receives the broadcast and
                        reads persisted sequence state. Deselecting removes
              ...
              >
                <header>
                  <h2>Release platforms</h2>
                  <p>
                    These credentials belong to build and release tooling, not
                    the deployed product runtime. Cloudflare is required for
                    Web; EAS, Apple and Google are needed only for the
                    corresponding mobile lanes; GitHub is optional automation.
                  </p>
                </header>
                <div className="provider-test-stack">
                  {(
                    [
                      {
                        ...
                        note: "Verifies the active token and exact account without deploying.",
                      },
                      {
                        id: "github-release",
                        name: "GitHub",
                        note: "Verifies the token's authenticated GitHub identity without changing a repository.",
                      },
                      {
                        id: "expo-eas",
                        name: "Expo / EAS",
                        note: "Verifies that EAS CLI resolves the exact configured project.",
                      },
                      {
                        id: "mobile-local-build",
                        name: "Local Android + Mac/Xcode",
                        note: "Verifies the local Android SDK and either local Xcode or an exact-commit connected Mac. EAS remains optional.",
                      },
                      {
                        id: "apple-app-store",
                        name: "Apple App Store Connect",
                        note: "Signs a short-lived API JWT and reads the exact configured app.",
                      },
                      {
                        id: "google-play",
                        name: "Google Play",
                        note: "Exchanges the service-account JWT and reads the configured Android app's subscriptions surface.",
                      },
                    ] satisfies Array<{
                      id: ReleaseProvider;
                      name: string;
                    ...
                    const test = releaseTests[id] || {
                      status: "idle" as const,
                    };
                    return (
                      <div className="storage-provider-config" key={id}>
                        <div>
                          <strong>{name}</strong>
                          <p>{note}</p>
                        </div>
                        <ProviderCredentialEditor
                          provider={id}
                          state={payload.providerCredentials[id]}
                          editing={providerEditors[id]}
                          values={providerSecrets}
                          onEditing={(editing) =>
                            setProviderEditing(id, editing)
                          }
                          onChange={updateProviderSecret}
                        />
                        <div className="provider-email-test">
                          <Button
                            ...
                            onClick={() => void runReleasePlatformTest(id)}
                          >
                            {test.status === "testing"
                              ? `Testing ${name}`
                              : `Test ${name}`}
              ...
              >
                <header>
                  ...
                  <p>
                    Select the providers this project will support. Credentials
                    may be inherited, entered now, or configured later from this
                    local Setup.
                  </p>
                </header>
                <div
                  className="provider-option-grid"
                  role="group"
                  aria-label="Social sign-in providers"
                >
                  {[
                    {
                      id: "google",
                      name: "Google",
                      note: "OAuth redirect flow for Web and Expo.",
                      href: providerSetupLinks.google[0].href,
                    },
                    {
                      id: "github",
                      name: "GitHub",
                      note: "OAuth sign-in with verified email access.",
                      href: providerSetupLinks.github[0].href,
                    },
                    {
                      id: "apple",
                      name: "Apple",
                      note: "Web redirect and native iOS audience support.",
                      href: providerSetupLinks.apple[0].href,
                    },
                    {
                      id: "microsoft",
                      name: "Microsoft",
                      note: "Microsoft Entra ID and personal accounts.",
                      href: providerSetupLinks.microsoft[0].href,
                    },
                    {
                      id: "discord",
                      name: "Discord",
                      note: "Community and consumer identity.",
                      href: providerSetupLinks.discord[0].href,
                    },
                    {
                      id: "facebook",
                      name: "Facebook",
                      note: "Meta OAuth application.",
                      href: providerSetupLinks.facebook[0].href,
                    },
                    {
                      id: "linkedin",
                      name: "LinkedIn",
                      note: "Professional identity provider.",
                      href: providerSetupLinks.linkedin[0].href,
                    },
                  ].map(({ id, name, note, href }) => {
                    const selected =
                      payload.blueprint.providers.socialAuth.includes(id);
                    return (
                      <div
                        className={
                          selected
                ...
                {payload.blueprint.providers.socialAuth.map((provider) => (
                  <div className="provider-test-stack" key={provider}>
                    ...
                    <div className="provider-live-test">
                      ...
                      <small>
                        This opens the real deployed OAuth flow. Saved
                        replacement credentials must be released to Development
                        before this test can validate them.
                      </small>
                    </div>
                  </div>
                ))}
              ...
              >
                <header>
                  ...
                  <p>
                    Email verification remains mandatory. Choosing “configure
                    later” defers only credentials and will remain a visible
                    release blocker.
                  </p>
                </header>
                ...
                <ProviderCredentialEditor
                  provider={
                    payload.blueprint.providers.email
                      .default as keyof typeof providerSecretFields
                  }
                  state={
                    payload.providerCredentials[
                      payload.blueprint.providers.email
                        .default as keyof SetupPayload["providerCredentials"]
                    ]
                  }
                  editing={
                    providerEditors[
                  ...
                  onChange={updateProviderSecret}
                />
                {new Set(["cfsend", "resend"]).has(
                  payload.blueprint.providers.email.default,
                ) ? (
                  <div className="provider-email-test">
                    <div>
                      <strong>Real delivery test</strong>
                      <p>
                        Send through the same adapter used by authentication
                        email. Entered replacement values are used for this test
                        without being returned by the server.
                      </p>
                    </div>
                    <label>
                    ...
                    >
                      {providerTest.status === "testing"
                        ? "Sending test email"
                        : `Send ${payload.blueprint.providers.email.default} test email`}
                    </Button>
                  ...
                  <p className="provider-test-unavailable">
                    Cloudflare Email Service needs its deployed Worker binding
                    and is tested after a Development release.
                  </p>
                )}
              </section>
              ...
              >
                <header>
                  ...
                  <p>
                    {billingSelected
                      ? "The selected Billing adapter is materialized. Stripe uses separate Test credentials for Development and Live credentials for Production; never reuse one environment's keys, webhook secret or Price ID in the other."
                      : "Billing is not selected and may be configured later."}
                  </p>
                </header>
                {billingSelected ? (
                  <>
                    <div
                      className="provider-option-grid"
                      role="radiogroup"
                      aria-label="Billing Provider"
                    >
                      {(["stripe", "polar", "autumn"] as const).map(
                        (provider) => (
                          <button
                            type="button"
                            className={
                            ...
                            aria-pressed={billingProvider === provider}
                            onClick={() => setBillingProvider(provider)}
                            key={provider}
                          >
                            <span>
                              <strong>
                              ...
                              <small>
                                {provider === "stripe"
                                  ? "Subscriptions, Checkout and Customer Portal."
                                  : provider === "polar"
                                    ? "Checkout, Portal, usage and signed Webhooks."
                                    : "Plans, usage and billing orchestration."}
                              </small>
                            </span>
                          </button>
                        ),
                    ...
                    <ProviderCredentialEditor
                      ...
                      values={providerSecrets}
                      onEditing={(editing) =>
                        setProviderEditing(billingProvider, editing)
                      }
                      onChange={updateProviderSecret}
                    />
                  </>
                ) : (
                  <div className="provider-default-card muted">
                    <div>
                      <strong>Billing not selected</strong>
                      <small>
                        Enable Billing & subscriptions in SaaS to choose Stripe,
                        Polar or Autumn.
                      </small>
                    </div>
                  </div>
              ...
              <details
                className="setup-panel provider-section provider-catalog-section"
                hidden={providerTab !== "advanced"}
              >
                <summary>
                  <span>
                    <strong>Advanced Provider catalog</strong>
                    <small>Inspect all 17 categories and Planned choices</small>
                  </span>
                  <span>Optional reference</span>
                </summary>
                <p>
                  Every common category is accounted for. Available options have
                  executable code and verification; Planned options remain
                  visible but disabled until their Pack is real.
                </p>
                    ...
                    <article key={category.id}>
                      <header>
                        <div>
                          ...
                          <small>
                            {category.kind} / {category.selection}
                          </small>
                        </div>
                        <span
                          className={`status ${category.required ? "defined" : "available"}`}
                        >
                      ...
                      <div className="provider-catalog-options">
                        {category.options.map((option) => (
                          <span
                            ...
                            className={
                              option.selectable
                                ? "provider-catalog-option available"
                                : "provider-catalog-option planned"
                            }
                            title={option.notes}
                          >
                            {option.name}
                            <small>
                              {option.selectable ? option.status : "planned"}
                            </small>
                          </span>
                        ))}
                      </div>
            ...
            <div className="setup-stack">
              {saveState === "saved" && !dirty ? (
                <section className="setup-panel setup-completion" role="status">
                  <span className="setup-completion-icon">
                    <Check size={22} />
                  </span>
                  <div>
                    <h2>Project plan saved</h2>
                    <p>
                      Your Blueprint and project configuration are persisted.
                      Nothing has been materialized or deployed automatically.
                    </p>
                  </div>
                  <div className="setup-completion-actions">
                    <Button asChild>
                      <a href="/dp">Continue independently</a>
                    </Button>
                    <Button asChild variant="outline">
                      <a href="/maintenance">
                        Connect All2CF for paid MCP and updates
                      </a>
                    </Button>
                    <Button
                      type="button"
              ...
              <section
                className="setup-panel review-panel"
                hidden={saveState === "saved" && !dirty}
              >
                ...
                <dl>
                  ...
                  <div>
                    <dt>Database</dt>
                    <dd>
                      {payload.blueprint.providers.database.initialState} Native
                      PostgreSQL / Hyperdrive from{" "}
                  ...
                  <div>
                    ...
                    <dd>
                      {payload.blueprint.providers.storage.provider} /{" "}
                      {payload.blueprint.providers.storage.access} /{" "}
                      {payload.blueprint.providers.storage.uploadMode}
                    </dd>
                  </div>
                  <div>
                    <dt>Anti-abuse</dt>
              ...
              <section
                className="setup-panel review-contracts"
                hidden={saveState === "saved" && !dirty}
              >
              ...
              {__STARTER_FACTORY_MODE__ || saveState !== "saved" || dirty ? (
                <Button
                  type="button"
                  className="save-blueprint"
                  onClick={() =>
                    void (__STARTER_FACTORY_MODE__
                      ? generateProject()
                      : save({ finish: true }))
                  }
                  disabled={
                    saveState === "saving" || generation.status === "generating"
                  }
                >
                  <Save size={16} />
          ...
          <footer className="setup-actions">
            ...
            {stepIndex < steps.length - 1 ? (
              <Button
                ...
                onClick={() =>
                  void save({
                    nextStepIndex: Math.min(steps.length - 1, stepIndex + 1),
                  })
                }
              >
