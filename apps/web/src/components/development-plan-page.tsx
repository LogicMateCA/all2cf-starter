...
type Snapshot = {
  ...
  assembly: {
    blueprint: {
      ...
      providers: { auth: string; socialAuth: string[]; database: DatabasePolicy; storage: StoragePolicy; antiAbuse: AntiAbusePolicy; email: { default: string; alternatives: string[] }; billing: string; release: string };
    };
      ...
      <div className="plan-content">
        ...
        <Section id="blueprint" title="Project Blueprint" description="The canonical selection record behind /setup, with realization tracked separately from intent." icon={Settings2}>
          <div className="assembly-summary">
            ...
            <Surface><span className="role-label">Provider defaults</span><h3>{data.assembly.blueprint.providers.auth}</h3><dl className="compact-facts"><div><dt>Social</dt><dd>{data.assembly.blueprint.providers.socialAuth.join(", ") || "none"}</dd></div><div><dt>Database</dt><dd>{data.assembly.blueprint.providers.database.provider} / {data.assembly.blueprint.providers.database.initialState}</dd></div><div><dt>Storage</dt><dd>{data.assembly.blueprint.providers.storage.provider} / {data.assembly.blueprint.providers.storage.access}</dd></div><div><dt>Anti-abuse</dt><dd>{data.assembly.blueprint.providers.antiAbuse.provider}</dd></div><div><dt>Email</dt><dd>{data.assembly.blueprint.providers.email.default}</dd></div><div><dt>Billing</dt><dd>{data.assembly.blueprint.providers.billing}</dd></div></dl></Surface>
