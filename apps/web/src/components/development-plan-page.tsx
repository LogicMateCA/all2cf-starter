...
type StoragePolicy = { provider: "none" | "cloudflare-r2" | "s3-compatible"; access: "private" | "public"; uploadMode: "worker"; maxUploadBytes: number; development: { bucket: string }; production: { bucket: string } };
type AntiAbusePolicy = { provider: "none" | "turnstile"; development: { siteKey: string }; production: { siteKey: string } };
type AiPolicy = { provider: "none" | "workers-ai"; development: { model: string; gatewayId: string }; production: { model: string; gatewayId: string } };
type SearchPolicy = { provider: "none" | "postgresql" | "vectorize"; development: { indexName: string; dimensions: number; metric: string }; production: { indexName: string; dimensions: number; metric: string } };
type PushPolicy = { provider: "none" | "expo-push"; accessTokenRequired: boolean; development: { projectId: string }; production: { projectId: string } };
type SmsPolicy = { provider: "none" | "twilio"; development: { apiBaseUrl: string }; production: { apiBaseUrl: string } };
type MediaPolicy = { images: { provider: "none" | "cloudflare-images"; maxInputBytes: number; defaultFormat: string }; stream: { provider: "none" | "cloudflare-stream"; maxDurationSeconds: number; development: { accountId: string; allowedOrigins: string[]; apiBaseUrl: string }; production: { accountId: string; allowedOrigins: string[]; apiBaseUrl: string } } };
...
type Snapshot = {
  ...
  assembly: {
    blueprint: {
      ...
      providers: { auth: string; socialAuth: string[]; database: DatabasePolicy; storage: StoragePolicy; antiAbuse: AntiAbusePolicy; ai: AiPolicy; search: SearchPolicy; push: PushPolicy; sms: SmsPolicy; media: MediaPolicy; background: BackgroundPolicy; email: { default: string; alternatives: string[] }; billing: string; release: string };
    };
    ...
    materialization: { schemaVersion: string; packs: Array<{ id: string; version: string; files: number }>; dependencyCount: number; generatedRoutesHash: string | null; generatedAuthServerHash: string | null; generatedAuthClientHash: string | null; generatedStorageAdapterHash?: string | null; generatedMobileConfigPluginsHash?: string | null; generatedDesignWebHash?: string | null; generatedDesignMarketingHash?: string | null; generatedDesignDocsHash?: string | null; generatedDesignMobileHash?: string | null; generatedMarketingProjectHash?: string | null };
  };
...
const fallback: Snapshot = {
  ...
  assembly: {
    blueprint: {
      ...
      providers: { auth: "better-auth", socialAuth: ["google"], database: { engine: "postgresql", provider: "native-postgresql", access: "sql-first", initialState: "empty", schemaSource: "selected-pack-baseline", existingDataPolicy: "out-of-scope", cfpg: { development: null, production: null } }, storage: { provider: "none", access: "private", uploadMode: "worker", maxUploadBytes: 10485760, development: { bucket: "starter-dev-objects" }, production: { bucket: "starter-objects" } }, antiAbuse: { provider: "none", development: { siteKey: "" }, production: { siteKey: "" } }, ai: { provider: "none", development: { model: "@cf/meta/llama-3.1-8b-instruct", gatewayId: "" }, production: { model: "@cf/meta/llama-3.1-8b-instruct", gatewayId: "" } }, search: { provider: "none", development: { indexName: "starter-dev-vectorize", dimensions: 768, metric: "cosine" }, production: { indexName: "starter-vectorize", dimensions: 768, metric: "cosine" } }, push: { provider: "none", accessTokenRequired: false, development: { projectId: "" }, production: { projectId: "" } }, sms: { provider: "none", development: { apiBaseUrl: "https://api.twilio.com" }, production: { apiBaseUrl: "https://api.twilio.com" } }, media: { images: { provider: "none", maxInputBytes: 10485760, defaultFormat: "image/webp" }, stream: { provider: "none", maxDurationSeconds: 600, development: { accountId: "00000000000000000000000000000000", allowedOrigins: ["dev.example.com"], apiBaseUrl: "https://api.cloudflare.com/client/v4" }, production: { accountId: "00000000000000000000000000000000", allowedOrigins: ["app.example.com"], apiBaseUrl: "https://api.cloudflare.com/client/v4" } } }, background: { cron: { enabled: false, development: { expression: "*/15 * * * *" }, production: { expression: "0 * * * *" } } }, email: { default: "cfsend", alternatives: ["resend", "cloudflare-email-service"] }, billing: "better-auth-stripe", release: "cloudflare-workers" },
    },
    ...
    materialization: { schemaVersion: "starter-materialization/v1", packs: [], dependencyCount: 0, generatedRoutesHash: null, generatedAuthServerHash: null, generatedAuthClientHash: null, generatedStorageAdapterHash: null, generatedMobileConfigPluginsHash: null, generatedDesignWebHash: null, generatedDesignMarketingHash: null, generatedDesignDocsHash: null, generatedDesignMobileHash: null, generatedMarketingProjectHash: null },
  },
  modules: [],
  changes: [],
  documents: [],
