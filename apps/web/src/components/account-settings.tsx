...
export function AccountSettings() {
  ...
  return <main className="product-main account-settings">
    ...
    <section className="settings-panel" aria-labelledby="preferences-heading"><div className="settings-panel-heading"><Laptop /><div><h2 id="preferences-heading">Preferences</h2><p>Stored per account and reused by the shared Product Shell.</p></div></div><div className="settings-row"><div><strong>Appearance</strong><p>System, light, or dark.</p></div><div className="segmented-control" role="group" aria-label="Theme">{(["system", "light", "dark"] as const).map((value) => <button key={value} aria-pressed={theme === value} onClick={() => setTheme(value)}>{value}</button>)}</div></div><div className="settings-row"><div><strong>Language</strong><p>Interface language for this account.</p></div><div className="segmented-control" role="group" aria-label="Language"><button aria-pressed={locale === "en"} onClick={() => setLocale("en")}>English</button><button aria-pressed={locale === "zh"} onClick={() => setLocale("zh")}>简体中文</button></div></div></section>
