...
- Better Auth owns random key generation, hashing, indexed verification, expiration, per-key database rate limiting, list, and revoke behavior. The plaintext key is returned once at creation and is not recoverable from list or storage.
- `user-keys` always belongs to the authenticated account. When Organizations is also selected, `org-keys` is added in the same official plugin: owners and admins manage shared keys, members can list them, and every request is scoped to the exact organization ID.
