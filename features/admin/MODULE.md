...
- Shared shell: `/admin`, `/support`, Dashboard and Settings use the same permanent responsive Product Shell, registered navigation, notification Bell and account entry. Visual Design owns their shared visual language; Admin modules must not invent page-local themes.
...
- Analytics & Scripts stores only external destination configuration, immutable revisions and Admin audit events. Cloudflare Web Analytics, Google Analytics, Google Tag Manager, Plausible and privileged custom snippets publish through one same-origin cached Loader. External sources require HTTPS; inline bodies are served from a same-origin per-integration JavaScript endpoint without `eval` or `unsafe-inline`. Starter does not store product pageviews, visitor sessions or analytics events. Admin/Auth/Setup/Maintenance remain excluded.
- Admin module navigation uses client-side history and keeps the shell mounted. Browser back/forward restores the selected module.
- Admin directories use one record per row for scanning. Complete user, organization and subscription controls open in a focused detail surface instead of expanding the list.
